...
class Praises(commands.Cog):
    ...
    async def add_praise(self, ctx, *args):
        ...
        if message == '':
            await ctx.reply("não esqueça de escrever o elogio a ser adicionado!")
        else:
            if message in praise_list:
                await ctx.reply("esse elogio já está na lista.")   
            else:
    ...
    @commands.command(name="elogios", help="mostra todas as formas de elogiar os outros")
